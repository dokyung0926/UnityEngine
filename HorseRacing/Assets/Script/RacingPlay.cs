...
public class RacingPlay : MonoBehaviour
{
    ...
    public void Register(PlayerMove playerMove)
    {
        ...
        totalPlayerNum++;
        Debug.Log($"{playerMove.gameObject.name}(이)가 등록 완료되었습니다 , 현재 총 등록 수 : {list_PlayerMove.Count}");
    }
    ...
    private void CheckPlayerReachedToGoalAndStopMove()
    {
        ...
        {
            // 1, 2, 3등은 단상에 위치시키고, 카메라는 단상을 향하도록 한다.
    ...
    {
        // 함수의  반환용 지연변수의 선언과 초기화는 함수의 가장 상단에 한다 .
        Transform tmpPlayerTransform = null;
        // 함수내용 : 연산에 따라 반환용 지연변수에 값을 대입한다 .
